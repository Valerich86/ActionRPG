...
public class GameManager : MonoBehaviour
{
    ...
    private IEnumerator Convert_int_to_SO(InventoryData data)
    {
        ...
        foreach (KeyValuePair<ItemSO, int> item in CurrentEquip)
        {
            Debug.Log($"Экипировка {item.Key} : {item.Value}");
        }
        foreach (KeyValuePair<ItemSO, int> item in CurrentLoot)
        {
            Debug.Log($"Лут {item.Key} : {item.Value}");
        }
